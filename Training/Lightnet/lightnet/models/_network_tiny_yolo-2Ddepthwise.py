...
class TinyYolo(lnn.module.Darknet):
    ...
    def __init__(self, num_classes=20, conf_thresh=.25, nms_thresh=.5, input_channels=3,
        ...
        layer_list = [
            ...
            OrderedDict([
                ...
                ('12_convdw',       lnn.layer.Conv2dDepthWise(int(alpha*512), int(alpha*1024), 3, 2, 1)),
            ]),

            OrderedDict([
                ('13_convbatch',    lnn.layer.Conv2dBatchReLU((4*64)+int(alpha*1024), 1024, 3, 1, 1)),
                ('14_conv',         nn.Conv2d(1024, len(self.anchors)*(5+self.num_classes), 1, 1, 0)),
            ])
        ]
    ...
    def _forward(self, x):
       ...
       # outputs.append(self.layers[2](outputs[0]))
        out = self.layers[1](torch.cat((outputs[1]), 1))
